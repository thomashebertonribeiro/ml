...
from src.models.category import CategoryDTO, CategoryDetailDTO
from src.services.exceptions import ApiError, ApiNetworkError, ApiRateLimitError
...
def _parse_category_dto(data: dict, parent_id: str | None = None, level: int = 0) -> CategoryDTO:
    """Converte um dict da API em CategoryDTO."""
    ...
    return CategoryDTO(
        ...
        parent_id=parent_id,
        level=level,
        total_items_in_this_category=data.get("total_items_in_this_category", 0),
...
def _parse_category_detail_dto(data: dict, parent_id: str | None = None, level: int = 0) -> CategoryDetailDTO:
    """Converte um dict detalhado da API em CategoryDetailDTO."""
    children = data.get("children_categories", [])
    children_ids = [c["id"] for c in children]
    path_from_root = data.get("path_from_root", [])
...
class MercadoLivreClient:
    ...
    - ``GET /sites/MLB/domain_discovery/search?q={query}`` — busca por texto
    ...
    def get_root_categories(self) -> list[CategoryDTO]:
        ...
        Returns:
            Lista de :class:`CategoryDTO` de nível 0.
    ...
    def _get_known_root_categories(self) -> list[CategoryDTO]:
    ...
    def search_categories(self, query: str) -> list[CategoryDTO]:
        """Busca categorias por texto livre.

        Estratégia em duas etapas:
        1. Tenta ``domain_discovery`` (limit máximo = 8 conforme API do ML)
        2. Se não encontrar resultados, faz busca local por nome nas
           categorias raiz e seus filhos diretos.

        Args:
            query: Termo de busca (ex.: "celulares").

        Returns:
            Lista de :class:`CategoryDTO` correspondentes (deduplicada).

        Raises:
            ApiError: Em caso de erro HTTP 4xx/5xx não recuperável.
            ApiNetworkError: Em caso de falha de rede.
        """
        results: list[CategoryDTO] = []

        # Etapa 1: domain_discovery (limit máximo aceito pela API é 8)
        try:
            url = f"{self.BASE_URL}/sites/MLB/domain_discovery/search"
            data = self._get(url, params={"q": query, "limit": 8})
            ...
            # 400 pode ocorrer com alguns termos — continua para etapa 2
            logger.warning("domain_discovery retornou erro (%s): %s", exc.status_code, exc)

        # Etapa 2: busca por nome nas categorias raiz se etapa 1 não retornou nada
        if not results:
            try:
                ...
                logger.warning("Busca nas categorias raiz falhou: %s", exc)

        # Deduplica por id mantendo a ordem
        seen: set[str] = set()
        unique: list[CategoryDTO] = []
        for cat in results:
    ...
    def get_category_detail(self, category_id: str) -> CategoryDetailDTO:
        ...
        Returns:
            :class:`CategoryDetailDTO` com ``children_ids`` preenchido.
        ...
        return _parse_category_detail_dto(data, parent_id=parent_id, level=level)
    ...
    def fetch_full_tree(
        ...
        Args:
            ...
            delay_ms: Intervalo em ms entre requisições (0 = sem delay).

        Returns:
            Lista plana de todos os :class:`CategoryDetailDTO` da árvore.
        """
        if visited is None:
            visited = set()

        result: list[CategoryDetailDTO] = []
